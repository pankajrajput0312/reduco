...
def main():
    ...
    images_to_use = []
    saved_path = []
    folder_path ="./data_depth_selection/depth_selection/test_depth_completion_anonymous/velodyne_raw"
    for img_name in os.listdir(folder_path):
        img_path = os.path.join(folder_path,img_name)
        saved_path_name = os.path.join("./output", img_name)
        saved_path.append(saved_path_name)
        images_to_use.append(img_path)
    # if "output" not in os.listdir("Kitti/depth/"):
        # os.mkdir("Kitti/depth/output")
    # os.mkdir("output")
    # for folder in os.listdir(folder_path):
    #     # os.mkdir("./output/"+folder)
    #     # os.mkdir("./output/"+folder+"/2011_09_26/")
    #     # os.mkdir("./output/"+folder+"/2011_09_26/"+folder+"/")

    #     inside_folder_path = folder_path + "/" + folder+"/2011_09_26/"+folder+"/"
    #     for img_folder in os.listdir(inside_folder_path):
    #         print(("/output/"+folder+"/2011_09_26/"+folder+"/"+img_folder))
    #         # os.mkdir("./output/"+folder+"/2011_09_26/"+folder+"/"+img_folder)
    #         print(inside_folder_path + img_folder)
    #         # os.mkdir("Kitti/depth/output/"+folder+"/proj_depth/groundtruth/"+img_folder)
    #         img_path = inside_folder_path + img_folder+"/data"
    #         try:
    #             print("............................................................","./depth_images/train/" + folder + "/proj_depth/groundtruth/"+img_folder)
    #             depth_image_folder = os.listdir("./depth_images/train/" + folder + "/proj_depth/groundtruth/"+img_folder)
    #             # print(depth_image_folder)
    #             for img in os.listdir(img_path):
    #                 # image_path = +"/"+img
    #                 # print(img)
    #                 if img in depth_image_folder:
    #                     # print("aaya")
    #                     depth_image_path = "./depth_images/train/" + folder + "/proj_depth/groundtruth/"+img_folder+"/"+img
    #                     images_to_use.append(depth_image_path)
    #                     saved_path.append("./output/"+folder+"/2011_09_26/"+folder+"/"+img_folder+"/"+img)
    #         except Exception:
    #             print("folder nahi hai")
    ...
    for i in range(num_images):
        ...
        if save_depth_maps:
            depth_image_file_name = depth_image_path.split("/")
            # print(depth_image_file_name)
            file_path = ""
            print(len(depth_image_file_name))
            # for i in range(8):
            #     if(i == 2):
            #         file_path = os.path.join(file_path, "output")
            #     else:
            #         file_path = os.path.join(file_path, depth_image_file_name[i])
            # cv2.imwrite(file_path, depth_image)
            # print("check.............................",saved_path[i])
            # with open(saved_path[i],'r') as f:
            #     print("badhiya") 
            depth_image = (final_depths * 256)
            # plt.imsave(saved_path[i],depth_image)
            # cv2.imwrite(saved_path[i], depth_image)
            # print(saved_path[i])
            # cv2.imwrite("/media/pankaj/Elements/ip_basic"+saved_path[i], depth_image)
            
            try:
                with open(saved_path[i], 'wb') as f:
